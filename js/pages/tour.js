...
(function () {
  ...
  window.TourPage = function () {
    var set = window.PointSet.random(8), message = null, rails = null;
    ...
    var page = window.Playground({
      ...
      fields: [
        ...
        { id: 'hint', kind: 'note', spacer: true, label:
          'Click the plane to add a point, click a point to remove it. At eight points or ' +
      ...
      onField: function (id, value, api) {
        ...
        if (id === 'clear') { set = window.PointSet(); message = 'Cleared. Click the plane to place points.'; return; }
      ...
      build: function (rail) {
        ...
        if (message) return { subject: set, gen: announce(message), title: 'Point plane' };
        if (tour) {
    ...
    page.surface.canvas.addEventListener('pointerdown', function (e) {
      ...
      if (hit != null) {
        set.removeAt(hit);
        message = 'Removed point ' + (hit + 1) + '. The rest were renumbered.';
      } else {
        var at = window.PointDraw.place(page.surface, px, py);
        set.add(at.x, at.y);
        message = 'Added point ' + set.count() + '. Press an algorithm to run it on the plane as it stands.';
      }
      page.rebuild();
    });
